...
OpenSky servers are slow and it will take a while to download the data. Unless they gant access to their Impala Shell service.
...
for AA in ARIP_LIST:                         # Outer loop is for each airport
    ...
    for d in global_index:                   # d is the date that is the end of the given period (1 week here)
        ...
        ## careful if running Arrivals db name should change to
        data_1.to_sql('DeparturesRaw', con=conn, if_exists='append')   # insert this week's data into SQL table
conn.close() 
