...
PASSWORD = "Centenus18!"
pl = pd.read_csv('C:/Users/Moses/Python/flights_data/airports_meta.csv') # airports db from data provider
...
MODE = 'departure' # 'departure'/'arrival'
...
conn = sqlite3.connect('flights_data_2.db') # db name
c = conn.cursor()
...
for AA in ARIP_LIST: #this loop is for airports. 60 airports 
    ...
    for d in global_index:
        ...
        flight_num = len(data_1) # number of flights in/out of the airport (based on departure/arrivals)
        global_df.loc[d,AA] = flight_num # if request failed puts -1
        data_1.to_sql('DeparturesRaw', con=conn, if_exists='append') #insert into SQL table)
# this is the csv map
conn.close()
filename_2 = 'C:/Users/Moses/Python/flights_data/' + 'all' + '_2' + MODE + '.csv'
